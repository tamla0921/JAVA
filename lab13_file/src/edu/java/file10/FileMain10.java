...
public class FileMain10 {
    ...
    public static void main(String[] args) {
        ...
        // System.getProperty(key): Java Runtime 실행 중인 컴퓨터의 여러가지 시스템 정보들을 리턴.
        // user.dir: 현재 작업 디렉토리(Current Working directory)
        // 프로그램이 실행되는 위치.
        ...
        System.out.println("D:\\study\\lab-java\\lab13_file\\src\\edu\\java\\file10"); // 경로를 복사하면, 이클립스에서는 자동으로 백슬래쉬를 \\로 바꿔 줌.
        System.out.println("data/utf8.txx"); // / 윈도우가 아닌 맥이나 리눅스에서 사용하는 경로 구분, 그러나 윈도우 자바에서는 자동으로 경로 구분으로 인식.
        ...
        File file = new File(path); // File 타입 객체 생성. --> 하드디스크에서의 유무는 모르겠지만, file이라는 객체를 프로그램에 만들음.
        
        System.out.println("path = " + file.getPath());
        // -> File 객체가 생성될 때 '상대 경로'로 생성된 경우, getPath() 메서드는 '상대 경로'를 리턴.
        
        
        
        File file2 = new File("D:\\study\\lab-java\\lab13_file");
        System.out.println("path = " + file2.getPath());
        // -> File 객체가 생성될 때 '절대 경로'로 생성된 경우, getPath() 메서드는 '절대 경로'를 리턴.
        
        System.out.println("절대 경로 = " + file.getAbsolutePath());
        System.out.println("파일 이름 = " + file.getName());
        System.out.println("폴더 이름 = " + file2.getName());
        // getName = 마지막 주소.? 유닉스에서는 폴더도, 파일도 모두 파일이라고 불렀기 때문에..
        
        System.out.println("파일? " + file.isFile());
        System.out.println("폴더? " + file.isDirectory());
        
        System.out.println("파일? " + file2.isFile());
        System.out.println("폴더? " + file2.isDirectory());
        
        System.out.println("파일1의 존재? " + file.exists());
        System.out.println("파일2(폴더)의 존재? " + file2.exists());
        
        System.out.println("파일 크기 = " + file.length()); // 바이트 단위
        ...
        for (File f : files) {
            ...
            System.out.println("파일명: " + f.getName()); //폴더와 파일의 혼재...
        }
        ...
        if (newFolder.exists()) {
            ...
            newFolder.mkdir(); // make directory의 약자
