...
// 1. [public static final] 필드(멤버 변수) // static이 아닌 필드는 정의할 수 없음. (상수만)
// 2. [public abstract] 메서드: 바디가 없는 추상 메서드

// 3. [public] default 메서드: Body({ ... })가 구현된 메서드. Java 8 버전부터.
//  --> 인터페이스를 구현하는 (하위) 클래스의 객체를 생성한 후에 참조 변수를 이용해서 호출(사용)하는 메서드.
// 인터페이스는 바디가 없는 것이 기본형, 바디가 없는 경우에는 메서드 앞에 default 키워드를 붙임.
...
// 6. private 메서드: body가 구현된 static 메서드. 자바 9 버전부터. default를 붙이지 않음.
